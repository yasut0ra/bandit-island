        ...
        {kind === "round" ? (
          <>
            <mesh position={[0, 0.45, 0]} castShadow>
              ...
              <meshStandardMaterial color="#5fbf5a" flatShading roughness={0.8} />
            </mesh>
            <mesh position={[0.32, 0.25, 0.1]} castShadow>
              <icosahedronGeometry args={[0.38, 0]} />
              <meshStandardMaterial color="#4eae4f" flatShading roughness={0.8} />
            </mesh>
            <mesh position={[-0.28, 0.3, -0.12]} castShadow>
              <icosahedronGeometry args={[0.34, 0]} />
              <meshStandardMaterial color="#6fcf62" flatShading roughness={0.8} />
            </mesh>
          </>
        ) : (
          <>
            {[0, 0.42, 0.78].map((y, i) => (
              <mesh key={y} position={[0, y + 0.2, 0]} castShadow>
                ...
                <meshStandardMaterial color={i === 1 ? "#3f9f6b" : "#48ad74"} flatShading roughness={0.85} />
              </mesh>
            ))}
          </>
        )}
...
function Rock({ position, scale = 1, rotation = 0 }: { position: [number, number, number]; scale?: number; rotation?: number }) {
  return (
    <mesh position={position} scale={scale} rotation={[0.3, rotation, 0.2]} castShadow receiveShadow>
      ...
      <meshStandardMaterial color="#a8a29e" flatShading roughness={1} />
    </mesh>
  );
}
...
const FLOWER_COLORS = ["#fda4af", "#fde68a", "#c4b5fd", "#ffffff", "#fdba74"];
  ...
  return (
    <group position={[HUB.x - 0.85, 0, HUB.z + 0.35]} rotation={[0, 0.35, 0]}>
      ...
      <mesh position={[0.18, 0.82, 0]} rotation={[0, 0, -0.08]} castShadow>
        ...
        <meshStandardMaterial color="#c08552" flatShading />
      </mesh>
      <mesh position={[0, 1.07, 0]}>
        <sphereGeometry args={[0.08, 10, 8]} />
        <meshStandardMaterial color="#fde68a" emissive="#fbbf24" emissiveIntensity={dark ? 2.5 : 0.6} />
      </mesh>
  ...
  return (
    <group>
      ...
      <mesh position={[0, -0.22, 0]} receiveShadow>
        ...
        <meshStandardMaterial color={dark ? "#5aa865" : "#8bd46e"} flatShading roughness={0.95} />
      </mesh>
      {/* grass lip */}
      <mesh position={[0, -0.5, 0]}>
        <cylinderGeometry args={[5.3, 5.05, 0.22, 16]} />
        <meshStandardMaterial color={dark ? "#468a52" : "#6fbf5d"} flatShading />
      </mesh>
      {/* earth layers */}
      <mesh position={[0, -1.05, 0]}>
        <cylinderGeometry args={[5.05, 4.4, 0.9, 12]} />
        <meshStandardMaterial color="#c98d5a" flatShading roughness={1} />
      </mesh>
      <mesh position={[0, -3.1, 0]} rotation={[Math.PI, 0.2, 0]}>
        <coneGeometry args={[4.4, 3.2, 10]} />
        <meshStandardMaterial color="#a86f45" flatShading roughness={1} />
      </mesh>
      <mesh position={[1.4, -2.6, 0.8]} rotation={[Math.PI, 0, 0.1]}>
        <coneGeometry args={[1.4, 2.2, 7]} />
        <meshStandardMaterial color="#94603c" flatShading roughness={1} />
      </mesh>
      <mesh position={[-1.8, -2.3, -0.6]} rotation={[Math.PI, 0, -0.1]}>
        <coneGeometry args={[1.2, 1.8, 7]} />
        <meshStandardMaterial color="#b77b4e" flatShading roughness={1} />
      </mesh>

      <Grass color={dark ? "#4f9a5e" : "#76c95f"} />
      <Flowers />
